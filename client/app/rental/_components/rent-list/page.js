...
export default function RentList() {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/api/rental/') // ✅ 改為從 API 讀取資料
      .then((response) => response.json())
      .then((data) => {
        if (data.success) {
          setRentals(data.data) // ✅ 這裡的 `data.data` 是 API 回傳的資料
          calculatePagination(data.data.length, itemsPerPage)
        } else {
          console.error('API 讀取失敗:', data.error)
        }
      })
